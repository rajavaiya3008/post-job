        ...
        {stepperDetail.map((stepper, i) => {
          return (
            <li
              ...
              key={i}
              onClick={() => dispatch(handleNavigate(i + 1))}
            >
              <span
                className={`absolute flex items-center justify-center w-8 h-8 ${
                  activeForm > i ? "bg-blue-500" : "bg-green-950"
                } rounded-full -start-4 ring-4 text-white ring-white`}
              >
                {activeForm > i + 1 ? "✓" : i + 1}
              </span>
              <h3 className="font-medium text-gray-900 pt-[4px]">
                {stepper?.name}
              </h3>
            </li>
          );
